...
export const ModConfig: React.FC = () => {
  ...
  const [imageAutoMod, setImageAutoMod] = useState(true);
  ...
  const loadConfig = async () => {
    ...
    if (config.image_auto_mod !== undefined) {
      setImageAutoMod(config.image_auto_mod);
    }
          ...
          <div className="pt-6 pb-24">
            ...
            >
              <div className="mb-8 w-full">
                ...
                {/* Image AutoMod Toggle */}
                <div className="mb-8 flex items-center justify-between p-4 border border-border/50 rounded-xl bg-muted/20">
                  <div>
                    <p className="text-sm font-medium text-foreground">Image Auto-Moderation</p>
                    <p className="text-xs text-muted-foreground mt-1">
                      When enabled, images are automatically checked via Google Vision API. When disabled, all image messages go to the manual review queue.
                    </p>
                  </div>
                  <button
                    role="switch"
                    aria-checked={imageAutoMod}
                    onClick={() => setImageAutoMod(prev => !prev)}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${imageAutoMod ? 'bg-libr-accent1' : 'bg-muted'
                      }`}
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${imageAutoMod ? 'translate-x-6' : 'translate-x-1'
                      }`} />
                  </button>
                </div>
