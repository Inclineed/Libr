...
func NewChatPeer(relayMultiAddrList []string) (*ChatPeer, error) {
	...
	if len(relayList) == 0 {
		return nil, fmt.Errorf("relay multiaddress list is empty")
	}
	...
	if len(distmap) == 0 {
		return nil, fmt.Errorf("failed to calculate distance for any relay")
	}
	...
	fmt.Println("Using relay:", relayIDused)
	...
	if relayAddr == "" {
		return nil, fmt.Errorf("could not find multiaddress for relay ID %s", relayIDused)
	}
...
func (cp *ChatPeer) Start(ctx context.Context) error {
	...
	if err != nil {
		fmt.Println("[DEBUG] Error opening stream to relay:", err)
		// Non-fatal: relay reservation is active, skip registration ping
		return nil
	}
	fmt.Println("[DEBUG] Opened stream to relay successfully")
	reqJson, err := json.Marshal(reqSent)
	if err != nil {
		fmt.Println("[DEBUG] Error marshalling register req:", err)
		stream.Close()
		return nil
	}
	if _, err := stream.Write([]byte(reqJson)); err != nil {
		fmt.Println("[DEBUG] Error writing register req:", err)
	}
...
func (cp *ChatPeer) Send(ctx context.Context, targetPeerID string, jsonReq []byte, body []byte) ([]byte, error) {
	...
	stream.Write(append(jsonReqRelay, '\n'))
	...
	if err != nil && err != io.EOF {
		fmt.Println("[DEBUG] Error reading response from relay:", err)
		return nil, err
	}
...
func XorHexToBigInt(hex1, hex2 string) *big.Int {
	...
	if err1 != nil || err2 != nil {
		log.Printf("Error decoding hex: %v %v\n", err1, err2)
		return nil
	}

	if len(bytes1) != len(bytes2) {
		log.Println("Hex strings must be the same length")
		return nil
	}
